...
200   CONTINUE
      RETURN
      END
            SUBROUTINE CKROP  (P, T, Y, ICKWRK, RCKWRK,prod,dest)
C     To calculate ROP which is used in path flux analysis
C     Get the normalized production-contributions and normalized destruction-contributions
C     In order to use this subroutine,you will call CKROP  (RWORK(NP), Z(1), Z(2), IWORK, RWORK,prod,dest)
C     Before call the subroutine, you should :Dimension prod(KK,II),dest(KK,II)
C  END PROLOGUE
C
C*****precision > double
        IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
...
C*****END precision > single
C
      DIMENSION ICKWRK(*), RCKWRK(*), Y(*), ROP(NKK,NII),NUKI(NKK,NII)
      DIMENSION Q(NII),prod(NKK,NII),dest(NKK,NII)
      INCLUDE 'ckstrt.h'
  ...
  101 CONTINUE   
  100 CONTINUE
      call CKQYP  (P, T, Y, ICKWRK, RCKWRK, Q)
      call CKNU   (NKK, ICKWRK, RCKWRK, NUKI)
C     to get production-contributions and destruction-contributions 
      do 102 I=1,NII
      do 103 K=1,NKK
      ROP(K,I) = NUKI(K,I) * Q(I)
  103 CONTINUE
  102 CONTINUE
C     write(*,*)'Q',Q
C     to get normalized production-contributions 
      do 104 K=1,NKK
      sum = 0.d0
      do 105 I=1,NII
      if (ROP(K,I) >= 0)THEN
...
C     write(*,*)'sum = ',sum
      do 106 I=1,NII
      prod(K,I) = max(ROP(K,I),0.D0) / sum
C     write(*,*)'prod = ',prod(K,I)
  106 CONTINUE
  104 CONTINUE 
c     to get normalized destruction-contributions 
      do 107 K=1,NKK
      sum = 0.d0
      do 108 I=1,NII
      if (ROP(K,I) <= 0)THEN
...
C     write(*,*)'sum = ',sum
      do 109 I=1,NII
      dest(K,I) = min(ROP(K,I),0.D0) / sum
C           write(*,*)'dest = ',dest(K,I)
  109 CONTINUE
  107 CONTINUE
      RETURN
      END
      ...
      SUBROUTINE CKcommon (ICKWRK, RCKWRK,NNREV,IIcRV,NNcTT,NNcAA,NNCP2,
     1                     NNCP2T,NNPAR,NNcCO,NNFAR,NNcFL,NNcLT,
     2                     IIcLT,NNcRL,IIcRL,NNLAN,NNLAR,NNRLT,NNcRV)
      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
      DIMENSION ICKWRK(*), RCKWRK(*)
      INCLUDE 'ckstrt.h'
      NNREV = NREV
