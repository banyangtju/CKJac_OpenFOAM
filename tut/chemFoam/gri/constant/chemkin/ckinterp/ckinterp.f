...
C@(#)                                            at 16:09:28
C@(#)  SCCS file name = /users/chemkin/SCCS/s.ckinterp.f
C@(#)===================================================================
C
      PROGRAM CKINTP
C
...
C     3.  Reduced the default lengths of KMAX and IMAX to something
...
C        EIM/val1/ - Electon-impct reaction; val1 is the integer
C                    temperature dependence flag
C        JAN/val1...val9/ - coefficients for electron reactions in
C                    the form of Jannev, Langer & Post:  
C                    k = SUM[an (lnT)^n]
C        FIT1/val1...val4/ - additional exponential terms for
C                    temperature powers > 1, e.g.,
C                    k = A T^B exp [ SUM (valn/T^n) ]
C        EXCI/val1/ - excitation reaction for energy los only;
C                     val1 is the energy loss per event in eV
C
...
C       NUNK(*,*) - array of species indices for reaction (MAXSP,IDIM)
...
C       ILAN(*)   - the NLAN reaction indices                   (IDIM)
C       PLAN(*,*) - Landau-Teller parameters               (NLAR,IDIM)
C
C       NREV      - number of reactions with reverse parameters
C       IREV(*)   - the NREV reaction indices                   (IDIM)
C       RPAR(*,*) - parameters                             (NPAR,IDIM)
C
C       NRLT      - number of reactions with reverse parameters
C                   and Landau-Teller parameters
C       IRLT(*)   - the NRLT reaction indices                   (IDIM)
C       RLAN(*,*) - reverse Teller-Laudauer parameters     (NLAR,IDIM)
C       NEIM      - number of reactions with electron impact
C       IEIM(*)   - the NEIM reaction indices                   (IDIM)
C       ITDEP(*)  - the NEIM temperature dependence flags       (IDIM)
C
C       NJAN      - number of Jannev, Langer, Evans & Post reactions 
C       IJAN(*)   - the NJAN reaction indices                   (IDIM) 
C       PJAN(*,*) - coefficients for the NJAN reactions    (NJAR,IDIM)
C
C       NFT1      - number of reactions using fit #1
C       IFT1(*)   - the NFT1 reaction indices                   (IDIM)
C       PFT1(*,*) - additional exponential terms for fit#1 (NF1R,IDIM)
C
C       NEXC      - number of excitation reactions
C       IEXC(*)   - the NEXC reaction indices                   (IDIM)
C       PEXC(*)   - energy loss per event in units of eV        (IDIM)
C
C       NRNU      - number of reactions having real stoichiometry
C       IRNU(*)   - the NRNU reaction indices                   (IDIM)
C       RNU(*,*)  - matrix of real stoich. coefficients   (MAXSP,IDIM)
C  
C       NORD      - number of reactions with modified species orders
C       IORD(*)   - the NORD reaction indices                   (IDIM)
C       KORD(*,*) - matrix of species indices whose order (MAXORD,IDIM)  
C                   is modified
C       RORD(*,*) - matrix of species order values        (MAXORD,IDIM)
C----------------------------------------------------------------------C
C*****precision > double
...
C
      PARAMETER (MDIM=10, KDIM=500, MKDIM=MDIM*KDIM, IDIM=2000, LSYM=16,
...
C**********************************************************************
C MARK O-01
      DATA NGRP / 0 /, NGDAT / 20*0 /, NELEM / 0 /
      DATA IGRP / 400*0 /
C**********************************************************************
C----------------------------------------------------------------------C
C
...
C     7                   NFT1, IFT1, NF1R, PFT1, NEXC, IEXC, PEXC)
C**********************************************************************
C MARK O-01
     7                   NFT1, IFT1, NF1R, PFT1, NEXC, IEXC, PEXC,
     8                   NELEM, NGRP, NGDAT, IGRP)
C**********************************************************************
C
         ELSE
C
...
C    3             NF1R, NFT1, NEXC, NRNU, NORD, MAXORD, CKMIN
C*****************************************************************
C MARK O-01
     3             NF1R, NFT1, NEXC, NRNU, NORD, MAXORD, CKMIN,
     4             NGRP, (NGDAT(NN), NN = 1, NGRP)
C*****************************************************************
...
C
C            NRNU, total number of reactions with real stoichiometry
C
     1      (IRNU(N), (RNU(M,N), M = 1, MAXSP), N = 1, NRNU)
C
C            IRNU, reaction indices
C            RNU,  matrix of real stoichiometric coefficients
C
...
C
C            IORD, reaction indices
C            KORD, array of species indices with "ORDER" specified,
C                  -K for forward species, K for reverse species 
C            RORD, array of order coefficients
C
C***********************************************************************
C MARK O-01
         IF (NGRP .GT. 0) WRITE (LINC)
C
C   NGRP, total number of reaction group
C
     1      ((IGRP(N,L), L=1, NGDAT(N)), N=1,NGRP)
C************************************************************************
C
      ELSE
         WRITE (LOUT, '(/A)')
...
C             NUNK  - the NSPEC species indices
C             NU    - the NSPEC stoichiometric coefficients 
C             NFAL  - total number of fall-off reactions
C             IFAL  - the NFAL reaction indices
C             KFAL  - 3rd body species indices for the NFAL reactions
C             NTHB  - total number of 3rd-body reactions
C             ITHB  - the NTHB reaction indices
C             NWL   - number of radiation-enhanced reactions
C             IWL   - the NWL reaction indices 
C             WL    - the NWL radiation wavelengths 
...
C
C    EIM/VAL1/:
C       if ITHB(NTHB)=I, this is an error, cannot have both
C       neutral 3rd-body dependence and e- impact
C
...
C             NKTB - array of species indices for the third body
C                         enchancement factors
C             NFAL - total number of fall-off reactions
C             IFAL - the NFAL reaction indices
C             IFOP - the NFAL fall-off types
C             PFAL - fall-off parameters
C             NLAN - total number of Landau-Teller reactions
C             ILAN - the NLAN reaction indices
...
C             IRLT - the NRLT reaction indices
C             RLAN - array of 'reverse' Landau-Teller numbers
C             NWL  - total number of radiation-enhanced reactions
C             IWL  - the NWL reaction indices
C             WL   - the NWL wavelengths
C             NEIM - total number of electron-impact reactions
C             IEIM - the NEIM reaction indices
C             ITDEP- the NEIM temperature dependence flags
C             NJAN - total number of Jannev, Langer, Evans & Post types
C             IJAN - the NJAN reaction indices
C             NJAR - number of coefficients required for J,L,E&P reacts
C             PJAN - array of coefficients of J,L,E&P reactions 
C             NFT1 - number of fit#1 reactions
C             IFT1 - the NFT1 reacton indices
C             PFT1 - array of added exponential parameters for fit#1
C             NEXC - number of excitation reactions
C             IEXC - the NEXC reaction indices
C             PEXC - the NEXC energy loss (eV)
C             NRNU - number of real stoichiometry reactions
C             IRNU - the NRNU reaction indices
C             RNU  - matrix of coefficients for the NRNU reactions
C             NORD - number of change-of-order reactions
C             IORD - the IORD reaction indices
C             KORD - matrix of species indices for the NORD reactions
...
C
C        ELECTRON IMPACT OR THIRD-BODY REACTIONS
C
            NEIM = NEIM + 1
            IEIM(NEIM) = II
            IF (LTHB) THEN
...
C*******************************************************************
C MARK O-01
         ELSEIF (UPCASE(KEY, 3) .EQ. 'GRO') THEN
C
C   DEFINITION OF REACTIONS' GROUP
C
            IF (UPCASE(RSTR,4) .EQ. 'FIRS') THEN
               NELEM = 0
               NGRP = NGRP + 1
               NELEM = NELEM + 1
               NGDAT(NGRP) = NELEM
               IGRP(NGRP, NGDAT(NGRP)) = II
               WRITE(LOUT,4200) NGDAT(NGRP),NGRP 
C
            ELSEIF (UPCASE(RSTR,4) .EQ. '****') THEN
               NELEM = NELEM + 1
               NGDAT(NGRP) = NELEM
               IGRP(NGRP, NGDAT(NGRP)) = II
               WRITE(LOUT,4200) NGDAT(NGRP), NGRP
C
            ELSEIF (UPCASE(RSTR,4) .EQ. 'LAST') THEN
               NELEM = NELEM + 1
               NGDAT(NGRP) = NELEM
               IGRP(NGRP, NGDAT(NGRP)) = II
               WRITE(LOUT,4200) NGDAT(NGRP),NGRP
...
C 1900 FORMAT (6X,A,T51,E10.3,F7.3,F11.3)
 ...
 3110 FORMAT (6X,'Jannev, Langer, Evans & Post type reaction:'
     1       /9X,'Coefficients: ',5(E10.3,1X)
     2      /23X,5(E10.3,1X))
 3112 FORMAT (6X,'Modified fit#1:  k= A * T^B * exp [SUM(Vn/T^n)]...',
     1       /9X,'Added parameters: ',E10.3,3(/27X,E10.3))
 3114 FORMAT (6X,'Excitation reaction only; Energy loss =',e10.3,' eV')
 4000 FORMAT (6X,'Declared duplicate reaction...')
 4100 FORMAT (6X,'Error in GROUP declaration...') 
 4200 FORMAT (6X,'Element No.', I2, 1X, 'of Group No.', I2)
      END
C----------------------------------------------------------------------C
...
C             KSPEC(N),N=1,MXSPEC- array of species indices in reaction
